	...
	if( isset( $_REQUEST['aksi'] )){
      ...
      echo '<div class="row">';
		...
		if(mysqli_num_rows($sql) > 0){
			$no = 1;
			while(list($tapel,$kelas,$jenis,$jumlah) = mysqli_fetch_array($sql)){
				echo '<tr><td>'.$no.'</td>';
				echo '<td>'.$tapel.'</td><td>'.$kelas.'</td><td>'.$jenis.'</td><td>Rp <span class="pull-right">'.number_format($jumlah).'</span></td><td>';
