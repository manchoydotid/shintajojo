...
if( empty( $_SESSION['iduser'] ) ){
	...
	if( isset( $_REQUEST['aksi'] )){
		...
		echo '<tr class="info"><th width="10px" style="text-align:center;">No</th><th width="60" style="text-align:center;">Kelas</th>';
		...
		if( mysqli_num_rows($sql) > 0 ){
			...
			while(list($kelas,$jumlah,$idrombel) = mysqli_fetch_array($sql)){
				...
				echo '<td><a href="./admin.php?hlm=master&sub=kelas&aksi=view&kelas='.$kelas.'&idrombel='.$idrombel.'&submit=y" class="btn btn-success btn-xs">View</a> ';
				echo '</tr>';
				$no++;
			}
		} else {
