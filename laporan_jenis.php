...
if (empty($_SESSION['iduser'])) {
    ...
    echo '<div class="col-md-7" style="margin: 0 auto;"><center><table class="table table-bordered" border="1" cellpading="10" >';
    ...
    if (mysqli_num_rows($sql) > 0) {
        ...
        while (list($id_jenis_bayar, $tapel, $kelas, $jenis, $jumlah) = mysqli_fetch_array($sql)) {
            echo '<tr><td>' . $no . '</td>';
            echo '<td>' . $tapel . '</td><td>' . $kelas . '</td><td>' . $jenis . '</td><td>Rp <span class="pull-right">' . number_format($jumlah) . '</span></td>';
            echo '</tr>';
            $no++;
        }
    } else {
