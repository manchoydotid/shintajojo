...
if( empty( $_SESSION['iduser'] ) ){
	...
	if( isset( $_REQUEST['aksi'] )){
		...
		$sql = mysqli_query($connect, "SELECT kelas.kelas, kelas.th_pelajaran, count(kelas.nis) as jml, siswa.idprodi FROM kelas,siswa WHERE kelas.nis=siswa.nis AND th_pelajaran='2014/2015' GROUP BY kelas.kelas");
		...
		if( mysqli_num_rows($sql) > 0 ){
			...
			while(list($kelas,$tapel,$jumlah,$idprodi) = mysqli_fetch_array($sql)){
				echo '<tr><td>'.$no.'</td>';
				echo '<td>'.$kelas.'</td>';
				echo '<td>'.$idprodi.'<span class="badge pull-right">'.$jumlah.' siswa</span></td>';
				echo '<td><a href="./admin.php?hlm=master&sub=kelas&aksi=view&kelas='.$kelas.'&tapel='.$tapel.'&idprodi='.$idprodi.'&submit=y" class="btn btn-success btn-xs">Edit</a> ';
