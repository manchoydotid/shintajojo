...
if( empty( $_SESSION['iduser'] ) ){
	...
	if( isset( $_REQUEST['submit'] )){
		...
		if(($submit=='simpan') AND isset($_REQUEST['nis'])){
			$nis = $_REQUEST['nis'];
			$sql = mysqli_query($connect,"INSERT INTO kelas VALUES('$kelas','$tapel','$nis')");
		}
		
		//proses hapus siswa dari kelas
		if(($submit=='hapus') AND isset($_REQUEST['nis'])){
			$nis = $_REQUEST['nis'];
			$qsiswa = mysqli_query($connect,"DELETE FROM kelas WHERE kelas='$kelas' AND th_pelajaran='$tapel' AND nis='$nis'");
		}
		...
		$qsiswa = mysqli_query($connect,"SELECT nis,nama FROM siswa WHERE idprodi='$idprodi' AND nis NOT IN (SELECT nis FROM kelas ) ORDER BY nis");
		...
		$qkelas = mysqli_query($connect,"SELECT nis FROM kelas WHERE kelas='$kelas' AND th_pelajaran='$tapel'");
...
<form method="post" action="admin.php?hlm=master&sub=kelas&aksi=view" class="form-horizontal" role="form">
	...
	<div class="form-group">
		...
		<div class="col-sm-3">
			...
			$qprodi = mysqli_query($connect,"SELECT * FROM prodi ORDER BY idprodi");
